...
using BotTom.Attributes;
using System;
using System.Threading.Tasks;
using Discord.WebSocket;
using System.Reflection.Metadata;

namespace BotTom.Modules;
...
public class ExampleModule : InteractionModuleBase<SocketInteractionContext>
{
    ...
    [ComponentInteraction("myButton:testButton")]
    ...
    public async Task GreetUserAsync(IUser user)
    {
        var cb = new ComponentBuilder()
            .WithButton(new ButtonBuilder()
                ...
                .WithLabel("Hello!")
                .WithDisabled(false)
                .WithCustomId("myButton:testButton")
            );
		...
		embedBuilder.WithTitle($":wave:");
		
		embedBuilder.WithColor(guildUser != null ? guildUser.Roles.First().Color : Color.Default);

		embedBuilder.WithDescription($"{( guildUser != null ? guildUser.DisplayName : Context.User.GlobalName)} said hi to you, <@{user.Id}>!");

		embedBuilder.WithCurrentTimestamp();

        await RespondAsync(embed: embedBuilder.Build(), components: cb.Build());
    }
